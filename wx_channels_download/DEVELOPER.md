...
- **日期区间**：
  - 开始日期：当天 00:00:00
  - 结束日期：当天 23:59:59
  - 支持多种日期格式：`2006.01.02`、`2006-01-02`、`2006/01/02`、`20060102`
- **解耦设计**：
...
- **相关文件**：
  - `pkg/credit/credit.go` - 积分管理核心逻辑（加密、解密、检查、消耗、日期解析）
...
后端解密积分数据
    ↓
检查：
    - 当前时间是否在有效区间内（StartAt <= now <= EndAt）
    - 积分是否足够（>= 5）
    ↓
...
```bash
# 生成 100 积分，有效期 2025.12.06 到 2025.12.13
go run main.go generate-credit --points 100 --start-date 2025.12.06 --end-date 2025.12.13

# 或使用横线格式
go run main.go generate-credit --points 200 --start-date 2025-12-06 --end-date 2025-12-13
...
**命令参数**：
- `--points`：积分数量（默认：1000）
- `--start-date`：开始日期（格式：`2006.01.02` 或 `2006-01-02`）
- `--end-date`：结束日期（格式：`2006.01.02` 或 `2006-01-02`）
...
2. 运行命令生成加密字符串：
   ...
   ```
3. 创建或编辑 `credit.yaml` 文件（与可执行文件同目录），填入生成的加密字符串：
   ```yaml
   encrypted: "生成的加密字符串"
   ```
4. 重启程序，积分功能自动启用
...
encrypted: c2kcuLHgcuxv9W3hAVJhQ7xouElo7pxanVMQyYZTC6qloXmRTfdUx9aY+ez/qtS14Agm98nM2QyCNjTayXVww1MqUIOpam1GlOgxmiyNiTWI/77y

或者直接创建文件：
echo encrypted: c2kcuLHgcuxv9W3hAVJhQ7xouElo7pxanVMQyYZTC6qloXmRTfdUx9aY+ez/qtS14Agm98nM2QyCNjTayXVww1MqUIOpam1GlOgxmiyNiTWI/77y > credit.yaml
...
- **生效检查**：当前时间 >= StartAt（开始日期 00:00:00）
- **过期检查**：当前时间 <= EndAt（结束日期 23:59:59）
- **有效区间**：[StartAt, EndAt]
